...
bool Values::Equals(const google_firestore_v1_Value& left,
  ...
  switch (left_type) {
    ...
    case TYPE_ORDER_STRING:
      return nanopb::MakeString(left.string_value) ==
             nanopb::MakeString(right.string_value);
    case TYPE_ORDER_BLOB:
      return left.bytes_value->size == right.bytes_value->size &&
             std::memcmp(left.bytes_value->bytes, right.bytes_value->bytes,
                         left.bytes_value->size) == 0;
...
bool Values::ObjectEquals(
  ...
  for (size_t i = 0; i < right_map.fields_count; ++i) {
    const std::string& key = nanopb::MakeString(left_map.fields[i].key);
...
int Values::CompareNumbers(const google_firestore_v1_Value& left,
                           const google_firestore_v1_Value& right) {
  if (left.which_value_type == google_firestore_v1_Value_double_value_tag) {
    ...
    if (right.which_value_type == google_firestore_v1_Value_double_value_tag) {
      return (int)(left_double - right.double_value);
    } else {
      return (int)(left_double - right.integer_value);
    }
  } else {
    long left_long = left.integer_value;
    if (right.which_value_type == google_firestore_v1_Value_integer_value_tag) {
      return left_long - right.integer_value;
    } else {
      return (int)(left_long - right.double_value);
    }
  }
}

int Values::CompareTimestamps(const google_firestore_v1_Value& left,
                              const google_firestore_v1_Value& right) {
  int cmp = left.timestamp_value.seconds - right.timestamp_value.seconds;
  if (cmp != 0) {
    return cmp;
  }
  return left.timestamp_value.nanos - right.timestamp_value.nanos;
}

int Values::CompareStrings(const google_firestore_v1_Value& left,
                           const google_firestore_v1_Value& right) {
  ...
  return left_string.compare(right_string);
}

int Values::CompareBlobs(const google_firestore_v1_Value& left,
                         const google_firestore_v1_Value& right) {
  size_t size = std::min(left.bytes_value->size, right.bytes_value->size);
  int cmp =
      std::memcmp(left.bytes_value->bytes, right.bytes_value->bytes, size) == 0;
  return cmp != 0 ? cmp : left.bytes_value->size - right.bytes_value->size;
}

int Values::CompareReferences(const google_firestore_v1_Value& left,
                              const google_firestore_v1_Value& right) {
  ...
  for (int i = 0; i < min_length; i++) {
    int cmp = left_segments[i].compare(right_segments[i]);
    if (cmp != 0) {
      return cmp;
    }
  }
  return left_segments.size() - right_segments.size();
}

int Values::CompareGeoPoints(const google_firestore_v1_Value& left,
                             const google_firestore_v1_Value& right) {
  int cmp =
      (int)(left.geo_point_value.latitude - right.geo_point_value.latitude);
  if (cmp != 0) {
    return cmp;
  }
  return (int)(left.geo_point_value.longitude -
               right.geo_point_value.longitude);
}

int Values::CompareArrays(const google_firestore_v1_Value& left,
                          const google_firestore_v1_Value& right) {
  int min_length =
      std::min(left.array_value.values_count, right.array_value.values_count);
  for (int i = 0; i < min_length; i++) {
    int cmp = Compare(left.array_value.values[i], right.array_value.values[i]);
    if (cmp) {
      return cmp;
    }
  }
  return left.array_value.values_count - right.array_value.values_count;
}

int Values::CompareObjects(const google_firestore_v1_Value& left,
                           const google_firestore_v1_Value& right) {
  google_firestore_v1_MapValue left_map = left.map_value;
  google_firestore_v1_MapValue right_map = right.map_value;
  ...
  while (left_it != left_key_to_value_index.end() &&
    ...
    int key_cmp = left_it->first.compare(right_it->first);
    if (key_cmp) {
      return key_cmp;
    }

    int value_cmp = Compare(left.map_value.fields[left_it->second].value,
  ...
  return left_it == left_key_to_value_index.end() &&
                 right_it == right_key_to_value_index.end()
             ? 0
             : (left_it == left_key_to_value_index.end() ? -1 : 1);
}
...
std::string Values::CanonifyReference(const google_firestore_v1_Value& value) {
  std::vector<std::string> segments = absl::StrSplit(
      nanopb::MakeString(value.reference_value), '/', absl::SkipEmpty());
  return absl::StrJoin(
