...
const std::string kTypeKey = "__type__";
const std::string kLocalWriteTimeKey = "__local_write_time__";
const std::string kServerTimestampSentinel = "server_timestamp";
  ...
  for (size_t i = 0; i < value.map_value.fields_count; ++i) {
    ...
    if (key == kTypeKey) {
      return value.map_value.fields[i].value.which_value_type ==
             ...
             nanopb::MakeString(value.map_value.fields[i].value.string_value) ==
